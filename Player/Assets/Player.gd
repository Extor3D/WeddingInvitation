...
onready var shoot_audio = $ShootPlayer
onready var hurt_anim = $AnimationPlayer
...
enum STATES {Idle, Walk, Jump, Fall, Climb, Glide, Defeated, Auto}
var state = STATES.Idle
var last_state = state

const bullet_scene = preload("res://Objects/PlayerBullet/PlayerBullet.tscn")
#func _ready():
#	check_abilities()
...
func _process(_delta):
	if state != STATES.Defeated and state != STATES.Auto:
		get_input()
...
func take_damage():
	if state != STATES.Defeated:
		hurt_anim.play("Damage")
		if hurt_audio.get_playback_position() == 0 || hurt_audio.get_playback_position() >= 0.1 || !hurt_audio.playing:
			hurt_audio.play()
		Global.take_damage()
		if Global.life <= 0:
...
func get_input(): 
	...
	if Input.is_action_just_pressed("shoot") && not shoot:
		try_shoot = true
		
	if try_shoot && can_shoot && state != STATES.Climb:
		shoot = true
...
#			stween.start()
	...
	sprite.position.x = 0
	if not $Shooting.is_stopped() && state != STATES.Climb:
...
func _on_Coyote_timeout():
	...
	if !on_ground && jump_left == jump_count && Global.difficulty == Global.DIFF.Hard:
